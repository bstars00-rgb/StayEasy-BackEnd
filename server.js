...
let memberships = [
  ...
  },
  {
    ...
    notes: 'Good all-rounder.', officialUrl: 'https://www.ihg.com/onerewards/', active: true,
  },
]
...
let voucherPacks = {
  'club-marriott-vietnam': [
    ...
    { templateId: 'cm-fnb50', category: 'discount', title: '50% F&B Discount', description: 'Dining discount.', quantity: 6, validUntil: '2026-11-30', hotels: [], city: null, transferable: false, note: 'Weekdays only.' },
    { templateId: 'cm-cake', category: 'gift', title: 'Birthday Cake', description: 'Birthday cake.', quantity: 1, validUntil: '2026-11-30', hotels: [], city: null, transferable: false, note: '48h pre-order.' },
  ],
  'accor-plus-vietnam': [
  ...
  'ihg-one-rewards-vietnam': [
    { templateId: 'ihg-dining20', category: 'dining', title: '20% Dining', description: 'Dining discount.', quantity: 6, validUntil: '2026-12-31', hotels: ['InterContinental Saigon'], city: 'ho-chi-minh', transferable: false, note: 'Weekdays only.' },
  ],
}
...
const TEMPLATE_OVERRIDES = {
  'cm-stay2': { minLeadDays: 3 }, 'cm-fnb50': { daysOfWeek: WEEKDAYS }, 'ap-dining50': { daysOfWeek: WEEKDAYS }, 'ihg-dining20': { daysOfWeek: WEEKDAYS }, 'cm-cake': { minLeadDays: 2 },
}
let holidays = [
...
function userState(userId) { if (!stateByUser.has(userId)) stateByUser.set(userId, { savedMemberships: [], usage: {}, reservations: [], orders: [], transfers: [] }); return stateByUser.get(userId) }
...
function walletFor(user) { const state = userState(user.id); const owned = state.savedMemberships.map((id) => getMembership(id)).filter(Boolean).map(publicMembership); const vouchers = state.savedMemberships.flatMap((membershipId) => getVoucherPack(membershipId).map((template) => voucherView(state, membershipId, template))); return { summary: { membershipCount: owned.length, availableVoucherCount: vouchers.reduce((s, v) => s + v.available, 0), expiringSoonCount: vouchers.filter((v) => v.available > 0 && daysUntil(v.validUntil) <= 30).length, openReservationCount: state.reservations.filter((r) => ['requested', 'confirmed'].includes(r.status)).length }, memberships: owned, vouchers, reservations: state.reservations, orders: state.orders, transfers: state.transfers } }
function createUser(credential) { const subject = ['demo-google-user', 'demo.user@gmail.com'].includes(String(credential)) ? 'demo-google-user' : String(credential || 'demo-google-user'); if (usersBySubject.has(subject)) return usersBySubject.get(subject); const email = subject === 'demo-google-user' ? 'demo.user@gmail.com' : `demo-${subject.slice(-8)}@stayeasy.local`; const user = { id: makeId('usr'), provider: 'google', providerSubject: subject, name: subject === 'demo-google-user' ? 'Demo User' : 'OhmySelect Demo User', email, picture: '', createdAt: now(), updatedAt: now() }; usersBySubject.set(subject, user); return user }
...
async function route(req, res) {
  ...
  if (req.method === 'GET' && path === '/') return send(res, 200, { ok: true, service: 'ohmyselect-backend', docs: '/api/v1/health' })
  if (req.method === 'GET' && path === '/health') return send(res, 200, { ok: true, service: 'ohmyselect-backend', time: now() })
  ...
  if (path.startsWith('/admin/')) {
    ...
    if (req.method === 'GET' && path === '/admin/memberships') { let rows = memberships.map(publicMembership); const q = (url.searchParams.get('q') || '').toLowerCase(), brand = url.searchParams.get('brand'); if (q) rows = rows.filter((m) => [m.id, m.name, m.brand].join(' ').toLowerCase().includes(q)); if (brand) rows = rows.filter((m) => m.brand === brand); return send(res, 200, paginate(rows, url.searchParams)) }
