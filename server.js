...
function voucherView(state, membershipId, template) { const used = state.usage[usageKey(membershipId, template.templateId)] || 0; const held = countOpenReservations(state, membershipId, template.templateId); const transferred = state.transfers.filter((t) => t.membershipId === membershipId && t.templateId === templateId).length; return { membershipId, ...template, used, held, transferred, available: Math.max(0, template.quantity - used - held - transferred) } }
...
function requireAdmin(req, res) { const user = requireUser(req, res); if (!user) return null; const isAdmin = adminEmails().has(String(user.email || '').toLowerCase()) || user.role === 'admin'; if (!isAdmin) { sendError(res, 403, 'ADMIN_REQUIRED', 'Admin access is required.'); return null } return { ...user, role: 'admin' } }
...
async function makePgPool(connectionString) { const dbUrl = new URL(connectionString); const originalHost = dbUrl.hostname; const [ipv4] = await resolve4(originalHost); return new Pool({ user: decodeURIComponent(dbUrl.username), password: decodeURIComponent(dbUrl.password), host: ipv4 || originalHost, port: Number(dbUrl.port || 5432), database: dbUrl.pathname.replace(/^\//, '') || 'postgres', ssl: process.env.PGSSLMODE === 'disable' ? false : { rejectUnauthorized: false, servername: originalHost } }) }
